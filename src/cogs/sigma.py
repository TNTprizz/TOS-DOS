...
class sigma(commands.Cog): # Show that it is a discord cog object
    ...
    async def queue(self, ctx, page = "NG"):
        if self.bot.playlist[ctx.guild.id]["np"] == "": # If nothing is playing
            nowp = "Nothing playing now\n"
        else: # If something is playing
            # Get the title and the duration of what is playing
            nowp = self.bot.songdes[self.bot.playlist[ctx.guild.id]["queue"][self.bot.playlist[ctx.guild.id]["order"] - 1]]["title"] + " - " + timeconvert(floor(time.time() - self.bot.playlist[ctx.guild.id]["startime"])) + "/"+ timeconvert(self.bot.songdes[self.bot.playlist[ctx.guild.id]["queue"][self.bot.playlist[ctx.guild.id]["order"] - 1]]["duration"])
